...
kind-check-db-setup: ## 🚀 통합 설정: Secrets → DB 확인 → 클러스터 생성 → 이미지 로드 (DB 제외)
	...
	@echo ""
	@echo "----------------------------------------------"
	@echo "  1단계: AWS 자격증명 확인 (ESO용)"
	@echo "----------------------------------------------"
	@echo ""
	@echo "ℹ️  시크릿은 External Secrets Operator(ESO)를 통해"
	@echo "   AWS SSM Parameter Store에서 자동 동기화됩니다."
	@echo ""
	@if [ -n "$$AWS_ACCESS_KEY_ID" ] && [ -n "$$AWS_SECRET_ACCESS_KEY" ]; then \
		echo "✅ AWS 환경변수 자격증명 확인됨"; \
		...
		echo "   클러스터 생성 시 ESO가 시크릿을 동기화할 수 없습니다."; \
		echo ""; \
	...
	@echo "=============================================="
	@echo ""
	@echo "  🔐 시크릿: AWS SSM Parameter Store (ESO 자동 동기화)"
	@echo ""
	@echo "  다음 단계:"
...
kind-localhost-setup: ## 🏠 통합 환경: 클러스터 생성 → 모든 이미지 로드 (DB + Frontend 포함)
	...
	@echo "  0. 필수 도구 확인 (istioctl)"
	@echo "  1. AWS 자격증명 확인 (ESO용)"
	@echo "  2. Kind 클러스터 생성 + Istio Ambient + ESO"
...
kind-dev-setup: ## 🔧 개발 환경: 클러스터 생성 → ECR 이미지 사용 (외부 DB 사용)
	...
	@echo "  2. AWS 로그인 확인 + ESO 자격증명"
	@echo "  3. Kind 클러스터 생성 + Istio Ambient + ESO"
	@echo "  4. 외부 DB 확인 + 연결 테스트 (172.18.0.1)"
	@echo "  5. 인프라 이미지 로드"
	@echo "  6. ECR 서비스 이미지 확인"
	@echo "  7. ArgoCD 설치 (선택)"
	@echo ""
	...
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [3/7] Kind 클러스터 생성"
	@echo "----------------------------------------------"
	@$(MAKE) kind-setup ENV=dev
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [4/7] 외부 DB 확인 + 연결 테스트"
	@echo "----------------------------------------------"
	@echo ""
	@echo "dev 환경은 호스트 PC의 PostgreSQL/Redis를 사용합니다."
	@echo ""
