...
# -----------------------------------------------------------------------------
kind-staging-setup: ## 🚀 Staging 환경: 클러스터 생성 → ArgoCD 자동 배포 (prod EKS처럼)
	@echo "=============================================="
	@echo "  weAlist Kind Staging 환경 설정"
	@echo "  (ArgoCD 자동 배포 + ESO)"
	@echo "=============================================="
	@echo ""
	@echo "이 명령어는 다음을 순서대로 실행합니다:"
	@echo "  1. 필수 도구 확인 (kubectl, kind, helm, istioctl, aws)"
	@echo "  2. AWS 로그인 확인"
	@echo "  3. Kind 클러스터 생성 + Istio Ambient + ECR Secret"
	@echo "  4. 인프라 이미지 로드"
	@echo "  5. ArgoCD 설치"
	@echo "  6. External Secrets Operator 설치"
	@echo "  7. ArgoCD Applications 자동 배포"
	@echo ""
	@echo "※ Staging 환경은 prod EKS처럼 동작합니다:"
	@echo "  - ArgoCD가 GitHub에서 설정을 읽어 자동 배포"
	@echo "  - ESO가 AWS Secrets Manager에서 시크릿 동기화"
	@echo "  - 외부 DB 사용 (Host 또는 AWS RDS)"
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [1/7] 필수 도구 확인"
	@echo "----------------------------------------------"
	@echo ""
	@# 도구 확인 (간소화)
	@command -v kubectl >/dev/null 2>&1 || { echo "❌ kubectl 필요"; exit 1; }
	@command -v kind >/dev/null 2>&1 || { echo "❌ kind 필요"; exit 1; }
	@command -v helm >/dev/null 2>&1 || { echo "❌ helm 필요"; exit 1; }
	...
	@if aws sts get-caller-identity >/dev/null 2>&1; then \
		...
		echo "❌ AWS 로그인 필요: aws sso login --profile <profile>"; \
		exit 1; \
	fi
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [3/7] Kind 클러스터 생성 + Istio + ECR Secret"
	@echo "----------------------------------------------"
	@echo ""
	@if kind get clusters | grep -q "wealist-staging"; then \
		echo "⚠️  wealist-staging 클러스터가 이미 존재합니다."; \
		read -p "삭제하고 다시 만들까요? (y/N): " answer; \
		if [ "$$answer" = "y" ] || [ "$$answer" = "Y" ]; then \
			kind delete cluster --name wealist-staging; \
		else \
			echo "기존 클러스터 사용"; \
		fi; \
	fi
	@# 클러스터 생성 (dev 스크립트 재사용, 클러스터 이름만 변경)
	@CLUSTER_NAME=wealist-staging ./k8s/helm/scripts/dev/0.setup-cluster.sh || \
		kind create cluster --name wealist-staging --config k8s/helm/scripts/dev/kind-config.yaml
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [4/7] 인프라 이미지 로드"
	@echo "----------------------------------------------"
	...
	@./k8s/helm/scripts/dev/1.load_infra_images.sh || echo "⚠️  일부 이미지 로드 실패 (무시 가능)"
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [5/7] ArgoCD 설치"
	@echo "----------------------------------------------"
	...
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [6/7] External Secrets Operator 설치"
	@echo "----------------------------------------------"
	@echo ""
	...
	@kubectl apply -f k8s/argocd/base/external-secrets/cluster-secret-store.yaml || \
		echo "⚠️  ClusterSecretStore 생성 실패 (AWS 권한 확인 필요)"
	@echo ""
	@echo "----------------------------------------------"
	@echo "  [7/7] ArgoCD Staging Applications 자동 배포"
	@echo "----------------------------------------------"
	@echo ""
	@$(MAKE) argo-deploy-staging
	@echo ""
	@echo "=============================================="
	@echo "  🎉 Staging 환경 설정 완료!"
	@echo "=============================================="
	@echo ""
	@echo "  ✅ 설치 완료:"
	@echo "    - Kind 클러스터 + Istio Ambient"
	...
	@echo "    - Staging Applications (자동 배포)"
	@echo ""
	@echo "  📝 ArgoCD 접속:"
	@echo "    URL: https://localhost:8079"
	@echo "    User: admin"
	...
	@echo ""
	@echo "  🔐 ESO 시크릿 설정:"
	@echo "    1. Terraform으로 AWS Secrets Manager에 값 저장"
	@echo "    2. ESO가 자동으로 K8s Secret 생성"
	@echo "    terraform/staging/foundation/secrets.tf 참조"
	@echo ""
	@echo "  🚀 배포 사이클:"
