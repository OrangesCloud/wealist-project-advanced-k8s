...
type S3Config struct {
	...
	AccessKey      string `yaml:"access_key"`      // MinIO only (optional)
	SecretKey      string `yaml:"secret_key"`      // MinIO only (optional)
	Endpoint       string `yaml:"endpoint"`        // Local MinIO - internal communication (optional)
	PublicEndpoint string `yaml:"public_endpoint"` // Public URL for browser access (optional)
}
