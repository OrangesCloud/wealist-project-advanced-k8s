...
func TestBadRequestWithDetails(t *testing.T) {
	router := setupTestRouter()
	router.GET("/test", func(c *gin.Context) {
		BadRequestWithDetails(c, "validation failed", "email is required")
	})
	...
	errorData := resp["error"].(map[string]interface{})
	assert.Contains(t, errorData["message"], "validation failed")
}

func TestValidationError(t *testing.T) {
	router := setupTestRouter()
	router.GET("/test", func(c *gin.Context) {
		ValidationError(c, "field 'email' is required")
	})
